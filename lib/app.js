...
const ADMIN_BAD = [];
const ADMIN_IDS = [...new Set([cfg('ADMIN_TELEGRAM_ID'), cfg('ADMIN_IDS'), '8808659518'].join(',').split(',')
  .map(x => x.trim()).filter(Boolean)
  .filter(x => /^\d{5,15}$/.test(x) || (ADMIN_BAD.push(x), false)).map(Number))];
/* адрес приложения. Не задан — на Vercel берём боевой домен проекта: он верный и после
   переименования проекта (так и случилось: apartments-medina → madinah-group-service) */
const APP_URL = (() => {
  const own = cfg('APP_URL');
  if (own) return /\/$/.test(own) ? own : own + '/';
  const prod = String(process.env.VERCEL_PROJECT_PRODUCTION_URL || '').trim();
  return prod ? 'https://' + prod.replace(/^https?:\/\//, '').replace(/\/+$/, '') + '/' : '';
})();
const APP_URL_FROM = cfg('APP_URL') ? 'APP_URL' : (APP_URL ? 'Vercel' : '');
/* Публичный Telegram-канал, из которого новые объявления автоматически попадают в каталог. */
const SOURCE_CHANNEL = String(cfg('SOURCE_CHANNEL', '@madinah_rent')).trim()
  .replace(/^https?:\/\/t\.me\//, '').replace(/^@/, '').toLowerCase();
const PORT = Number(cfg('PORT', 8443));
/* пароль вебхука: без него любой, кто знает адрес, мог бы прислать «нажатие кнопки» от имени
   администратора. Не задан — выводим из токена бота: у Telegram и у нас он один и тот же. */
const WEBHOOK_SECRET = cfg('WEBHOOK_SECRET') ||
  (TOKEN ? crypto.createHmac('sha256', TOKEN).update('madinah-webhook').digest('hex').slice(0, 48) : '');
...
async function readAll() {
  ...
  /* При выпуске новой редакции условий переносим оферту в сохранённый контент,
     чтобы существующая база не продолжала показывать старую редакцию. */
  const storedOfferVersion = Number(CONTENT.offer && CONTENT.offer.version) || 0;
  const baseOfferVersion = Number(DATA.offer && DATA.offer.version) || 0;
  if (baseOfferVersion > storedOfferVersion) {
    CONTENT.offer = JSON.parse(JSON.stringify(DATA.offer));
...
function ensureWebhook() {
  ...
  if (SETTINGS.menuButtonVersion !== 'v1' || SETTINGS.menuButtonUrl !== menuUrl.toString() || SETTINGS.menuButtonText !== '🏠 Madinah Group') {
  ...
  // переставляем, если сменился адрес ИЛИ пароль (сам пароль в базе не храним — только отпечаток)
  const sig = crypto.createHash('sha256').update(target + '|' + WEBHOOK_SECRET + '|updates-v4').digest('hex').slice(0, 16);
  if (SETTINGS.webhookSig === sig) return;
  if (SETTINGS.webhookTry && Date.now() - SETTINGS.webhookTry < 600000) return;    // после сбоя — не чаще раза в 10 минут
  SETTINGS.webhookTry = Date.now();
  saveSettings();
  track(tg('setWebhook', Object.assign({
    url: target, allowed_updates: ['message', 'callback_query', 'channel_post'], max_connections: 1
  }, WEBHOOK_SECRET ? { secret_token: WEBHOOK_SECRET } : {})).then(() => {
    SETTINGS.webhookUrl = target;
    SETTINGS.webhookSig = sig;
    SETTINGS.webhookTry = 0;
    saveSettings();
...
function tg(method, payload) {
  if (!TOKEN) { const no = Promise.reject(new Error('нет токена')); no.catch(() => {}); return no; }
  ...
  /* На Vercel функция засыпает сразу после ответа: запрос в Telegram, которого никто не ждёт,
     просто не успевает уйти. Поэтому каждый вызов записываем в TASKS — handle() дождётся его
     до ответа клиенту. Пустой catch — чтобы ошибка «ничейного» вызова не роняла процесс. */
  p.catch(() => {});
  track(p);
  return p;
}
...
function send(chatId, msg, extra) {
  const p = sendNow(chatId, msg, extra);
  p.catch(() => {});                  // вызвавший мог не ждать ответа — ошибка уже записана в лог
  return p;
}
async function sendNow(chatId, msg, extra) {
...
const GREET = {
  ru: ['<b>Что умеет этот бот?</b>', '',
       ...
       '🇷🇺 <b>Русский</b>',
       '<b>Madinah Group — сервис в Медине</b>',
       '🏠 Квартиры: фото, видео, цены и локация.',
       '🕌 Визы, зиярат, туры и аренда авто.',
       '📱 Бронирование и заявки — в приложении.',
       '🕐 24/7',
       '👇 Откройте каталог кнопкой ниже.',
       '',
       '🇺🇸 <b>English</b>',
       'Madinah Group — Service in Madinah',
       '🏠 Apartments: photos, videos, prices & locations.',
       '🕌 Visas, Ziyarat, tours & car rentals.',
       '📱 Book and request services in the app.',
       '🕐 24/7',
       '👇 Open the catalogue below.'],
  uz: ['<b>Bu bot nimalarni biladi?</b>', '',
       'Assalomu Alaykum va rahmatullohi va barokatuhu',
       '🇺🇿 <b>O‘zbekcha</b>',
       '<b>Madinah Group — Madinadagi xizmat</b>',
       '🏠 Kvartiralar: suratlar, videolar, narxlar va lokatsiya.',
       '🕌 Vizalar, ziyoratlar, turlar va avtomobil ijarasi.',
       '📱 Bron qilish va xizmatlarga ariza — ilovada.',
       '🕐 24/7',
       '👇 Katalogni quyidagi tugma orqali oching.',
       '',
       '🇺🇸 <b>English</b>',
       'Madinah Group — Service in Madinah',
       '🏠 Apartments: photos, videos, prices & locations.',
...
async function queueImport(msg, options) {
  const directChannel = !!(options && options.directChannel);
  try {
    await importOne(msg, { directChannel });
  } catch (e) {
    console.error('импорт:', e.message);
    if (!directChannel) await send(msg.chat.id, '❌ Не получилось: ' + e.message);
    else ADMIN_IDS.forEach(id => send(id, '❌ Автоимпорт из @' + SOURCE_CHANNEL + ': ' + escapeHtml(e.message)));
  }
}
...
async function importOne(msg, options) {
  const directChannel = !!(options && options.directChannel);
  const chatId = msg.chat.id;
  ...
  if (first) {
    store.sweep('album:', 6);
    if (!directChannel && (msg.photo || msg.video || msg.document)) await send(chatId, '⏳ Забираю фото и видео в полном качестве…');
  }

  const got = await grabMedia(msg);
  const text0 = msg.caption || msg.text || '';
  const origin = msg.forward_origin || {};
  const forwardChat = origin.chat || msg.forward_from_chat || {};
  const sourceChat = directChannel ? (msg.chat || {}) : forwardChat;
  const postId = directChannel ? msg.message_id : (origin.message_id || msg.forward_from_message_id);
  const postUrl = sourceChat.username && postId ? `https://t.me/${sourceChat.username}/${postId}` : '';
  ...
  if (ap) {
    ...
    if (first) {
      if (!directChannel) {
        await send(chatId, '✅ Обновил «' + (ap.title.ru || ap.id) + '»: фото ' + got.photos.length + ', видео ' + got.videos.length + ' — теперь в полном качестве.', editKeyboard(ap.id));
      } else {
        ADMIN_IDS.forEach(id => send(id, '✅ @' + SOURCE_CHANNEL + ': объявление «' + (ap.title.ru || ap.id) + '» уже было в каталоге — медиа обновлены.', editKeyboard(ap.id)));
      }
    }
    if (got.big) {
      const bigNote = '⚠️ Видео больше 20 МБ Telegram ботам не отдаёт. Добавьте его в приложении: карточка → «Редактировать» → «Добавить».';
      if (!directChannel) await send(chatId, bigNote); else ADMIN_IDS.forEach(id => send(id, bigNote));
    }    return;
  }
  ...
  const draft = sanitize({
    code: p.code, status: directChannel ? (p.status || 'free') : 'draft', district: district,
  ...
  /* Для прямой публикации в целевом канале карточка сразу становится видимой. */
  if (directChannel && !draft.title.ru && !draft.title.uz) {
    draft.title = { ru: draft.code ? 'Квартира ' + draft.code : 'Новая квартира', uz: '', en: '' };
  }
  LISTINGS.push(draft);
  saveListings();
  ...
  if (!directChannel) {
    await send(chatId,
      '📝 Черновик создан: «' + (draft.title.ru || 'без названия') + '»\n' +
      ...
      'Проверьте и нажмите «Сохранить и показать» — до этого черновик видите только вы.',
      editKeyboard(draft.id)
    );
  } else {
    ADMIN_IDS.forEach(id => send(id,
      '🏠 @' + SOURCE_CHANNEL + ': «' + (draft.title.ru || draft.id) + '» автоматически добавлена в каталог.\n' +
      'Фото: ' + got.photos.length + ', видео: ' + got.videos.length + '. Разобрано полей: ' + p.filled.length + '.',
      editKeyboard(draft.id)
    ));
  }
  if (got.big) {
...
function isSourceChannel(msg) {
  return !!(msg && msg.chat && msg.chat.type === 'channel' &&
    String(msg.chat.username || '').toLowerCase() === SOURCE_CHANNEL);
}

async function handleUpdate(u) {
  try {
    if (u.message) await handleMessage(u.message);
    else if (u.channel_post && isSourceChannel(u.channel_post)) await queueImport(u.channel_post, { directChannel: true });
...
const hits = new Map();
function tooMany(req, max, windowMs, key) {
  const ip = String(req.headers['x-forwarded-for'] || (req.socket && req.socket.remoteAddress) || '').split(',')[0].trim();
  const k = ip + '|' + (key || '');                  // у диагностики свой счётчик, у заявок — свой
  const now = Date.now();
...
/* Проверка связи с Telegram по шагам — ничего не отправляет:
   токен (getMe) → вебхук (getWebhookInfo) → адрес приложения → может ли бот писать админу
   (sendChatAction: у админа на пару секунд появится «печатает…», сообщения не будет). */
async function telegramCheck(req) {
  const out = { telegram: {} };
  const t = out.telegram;
  if (!TOKEN) { t.token = 'нет BOT_TOKEN'; return out; }
  let me = null;
  try { me = await tg('getMe', {}); t.token = 'ok'; t.bot = '@' + me.username; }
  catch (e) { t.token = 'ошибка: ' + e.message + ' — проверьте BOT_TOKEN (без кавычек и пробелов)'; return out; }
  const host = String(req.headers['x-forwarded-host'] || req.headers.host || '').split(',')[0].trim();
  let appHost = '';
  try { appHost = new URL(APP_URL).host; } catch (e) {}
  t.appUrl = APP_URL ? APP_URL + (APP_URL_FROM === 'Vercel' ? ' (взят из настроек Vercel)' : '') : 'не задан';
  t.appUrlOk = !!appHost && appHost === host;
  if (!t.appUrlOk) t.appUrlHint = 'APP_URL должен быть https://' + host + '/ — иначе вебхук и кнопка Mini App ведут не туда';
  try {
    const w = await tg('getWebhookInfo', {});
    t.webhook = w.url || 'не установлен';
    t.webhookOk = !!w.url && w.url.indexOf(host) >= 0;
    if (w.last_error_message) t.webhookLastError = w.last_error_message;
    t.webhookAllowedUpdates = w.allowed_updates || [];
    t.pending = w.pending_update_count || 0;
  } catch (e) { t.webhook = 'ошибка: ' + e.message; }
  t.admins = await Promise.all(ADMIN_IDS.map(async (id, i) => {
    try { await tg('sendChatAction', { chat_id: id, action: 'typing' }); return 'админ ' + (i + 1) + ': ok'; }
    catch (e) {
      const m = String(e.message || e);
      return 'админ ' + (i + 1) + ': ' + (/initiate|chat not found|blocked/i.test(m)
        ? 'бот не может ему писать — администратор должен открыть бота и нажать «Старт» (' + m + ')' : m);
    }
  }));
  if (!ADMIN_IDS.length) t.admins = ['ADMIN_TELEGRAM_ID не задан'];

  /* Диагностика автоимпорта: бот должен видеть исходный канал. */
  try {
    const source = await tg('getChat', { chat_id: '@' + SOURCE_CHANNEL });
    t.sourceChannel = '@' + SOURCE_CHANNEL + ' (' + source.id + ')';
    try {
      const member = await tg('getChatMember', { chat_id: source.id, user_id: me.id });
      t.sourceChannelMember = member.status || 'unknown';
      if (member.status === 'left' || member.status === 'kicked') {
        t.sourceChannelHint = 'Добавьте @' + me.username + ' в канал @' + SOURCE_CHANNEL + ' (лучше как администратора).';
      }
    } catch (e) {
      t.sourceChannelMember = 'ошибка: ' + String(e.message || e);
      t.sourceChannelHint = 'Добавьте @' + me.username + ' в канал @' + SOURCE_CHANNEL + ' (лучше как администратора).';
    }
  } catch (e) {
    t.sourceChannel = 'ошибка: ' + String(e.message || e);
    t.sourceChannelHint = 'Проверьте, что канал @' + SOURCE_CHANNEL + ' существует и бот добавлен в него.';
  }

  t.menuButton = SETTINGS.menuButtonVersion === 'v1' ? (SETTINGS.menuButtonUrl || 'ok') : 'не установлена';
  return out;
}
  ...
  try {
    ...
    if (p === '/api/health') {
      ...
      if (url.searchParams.get('telegram') === '1') {
        if (tooMany(req, 6, 60000, 'diag')) return json(res, 429, { ok: false, error: 'Слишком часто, подождите минуту' });
        return json(res, 200, Object.assign({ ok: true, ready, store: store.kind, media: media.kind }, await telegramCheck(req)));
      }
      return json(res, 200, { ok: true, app: 'madinah-rent', dev: DEV, ffmpeg: media.hasFfmpeg, bot: !!TOKEN,
        ...
        maxUploadMb: Math.round(MAX_UPLOAD / 1048576), ready, telegramMenu: SETTINGS.menuButtonVersion === 'v1', error: why });
    }
    ...
    if (p === '/api/request' && req.method === 'POST') {
      ...
      REQUESTS.push(r);
      saveRequests();
      const telegram = await notifyRequest(r);
      if (!telegram.ok) {
        // заявка сохранена, но до Telegram не дошла — приложение предложит отправить её вручную
        return json(res, 502, { ok: false, telegram: false, error: 'Не удалось отправить заявку в Telegram' });
      }
...
async function notifyRequest(r) {
  /* все отправки собираем, чтобы знать, дошла ли заявка хоть до кого-то */
  const sent = [];
  const out = (chatId, msg, extra) => { const p = send(chatId, msg, extra); sent.push(p); return p; };
  const result = async () => {
    const res = await Promise.allSettled(sent);
    const delivered = res.filter(x => x.status === 'fulfilled').length;
    return { ok: delivered > 0, sent: delivered, failed: res.length - delivered };
  };
  ...
  if (r.kind === 'visa') {
    ...
    partners.forEach(pt => out(pt.id, body(pt.lang), kb(pt.lang, [PARTNER.done[pt.lang] || PARTNER.done.ru, PARTNER.no[pt.lang] || PARTNER.no.ru])));
    ...
    ADMIN_IDS.forEach(id => out(id, copy, partners.length ? (r.user ? { reply_markup: { inline_keyboard: [[{ text: '✍️ Написать клиенту', url: 'tg://user?id=' + r.user.id }]] } } : {})
  ...
  if (r.kind === 'transfer') {
    ...
    ADMIN_IDS.forEach(id => out(id, lines.join('\n'), kb('ru', ['✅ Договорились', '✖ Отказ'])));
    return result();
  }
...
module.exports = {
  ...
  store, media, ENV, cfg, DEV, HOSTED, TOKEN, APP_URL, ADMIN_IDS, PORT, WEBHOOK_SECRET,
