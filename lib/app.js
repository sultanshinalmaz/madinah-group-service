...
Проверьте и нажмите «Сохранить и показать» — до этого черновик видите только вы.', editKeyboard(draft.id));
    ...
    ADMIN_IDS.forEach(id => send(id, '🏠 @' + SOURCE_CHANNEL + ': «' + (draft.title.ru || draft.id) + '» автоматически добавлена в каталог.
Фото: ' + got.photos.length + ', видео: ' + got.videos.length + '. Разобрано полей: ' + p.filled.length + '.', editKeyboard(draft.id)));
  }
  if (got.big) {
    const bigNote = '⚠️ Видео больше 20 МБ Telegram ботам не отдаёт. Добавьте его в приложении: карточка → «Редактировать» → «Добавить».';
    if (!directChannel) await send(chatId, bigNote); else ADMIN_IDS.forEach(id => send(id, bigNote));
