...
const TAB_IDS = ['catalog', 'visa', 'tours', 'cars', 'offer'];
...
/* ------------------------------------------------------ сделки для отчёта */
const KINDS = ['housing', 'visa', 'tour', 'car', 'other'];
...
function reportData(month) {
  ...
  return {
    ...
    counts: {
      ...
      car: req.filter(r => r.kind === 'car').length
    },
    months: [...months].sort()
  };
}
...
function reportText(month) {
  ...
  return [
    ...
    `Заявки: жильё ${r.counts.bookings} (подтверждено ${r.counts.confirmed}) · визы ${r.counts.visa} · туры ${r.counts.tour} · авто ${r.counts.car}`,
...
function tg(method, payload) {
  ...
  return new Promise((resolve, reject) => {
    ...
    const req = https.request({
      ...
      res.on('data', d => out += d);
      res.on('end', () => {
        try { const j = JSON.parse(out); j.ok ? resolve(j.result) : reject(new Error(j.description)); }
        catch (e) { reject(e); }
      });
    });
    req.on('error', reject);
...
function send(chatId, msg, extra) {
  ...
  return tg('sendMessage', Object.assign({ chat_id: chatId, text: msg, parse_mode: 'HTML', disable_web_page_preview: true }, extra || {}))
...
async function handleMessage(msg) {
  ...
  if (!isAdmin) {
    ...
    await send(chatId, hint + DATA.contacts.brothers.tg, appKeyboard(lang));
    return;
  }
...
async function route(req, res) {
  ...
  try {
    ...
    if (p === '/api/booking' && req.method === 'POST') {
      ...
      saveBookings();
      notifyBooking(booking, ap);
      if (user) send(user.id, CLIENT.sent[booking.lang]);
    ...
    if (p === '/api/request' && req.method === 'POST') {
      ...
      const b = await readBody(req);
      const user = verifyInitData(req.headers['x-init-data'] || '');
      const kind = ['visa', 'tour', 'car'].includes(b.kind) ? b.kind : '';
