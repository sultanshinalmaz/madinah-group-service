...
const BLOCK_IDS = ['housingMap', 'housingFav', 'offerAbout', 'offerSteps', 'offerCases', 'offerContacts', 'visaDocs', 'visaPriceNote', 'badge247', 'carsTransfer'];
  ...
  try {
    ...
    if (p === '/api/request' && req.method === 'POST') {
      ...
      const r = {
        ...
        bags: b.bags === 'no' ? 'no' : 'yes', kids: b.kids === 'yes' ? 'yes' : 'no', chair: b.chair === 'yes' ? 'yes' : 'no',
...
function notifyRequest(r) {
  ...
  if (r.kind === 'transfer') {
    ...
    const lines = [
      ...
      `<b>Багаж:</b> ${r.bags === 'no' ? 'без багажа' : 'есть'} · <b>Дети:</b> ${yn(r.kids)}`,
