...
function ensureWebhook() {
  ...
  const sig = crypto.createHash('sha256').update(target + '|' + WEBHOOK_SECRET + '|updates-v3').digest('hex').slice(0, 16);
...
async function telegramCheck(req) {
  ...
  try { const me = await tg('getMe', {}); t.token = 'ok'; t.bot = '@' + me.username; }
